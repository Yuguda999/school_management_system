...
const SubjectAttendanceRecordsPage: React.FC = () => {
    ...
    const fetchAttendanceRecords = async () => {
        ...
        try {
            ...
            // Ensure we are fetching subject attendance (where subject_id is NOT null)
            // If no specific subject selected, we still want only records that HAVE a subject
            // The backend will handle this if we pass a flag or if we just filter on client side
            // For now, let's assume the backend returns all and we filter, OR we can enforce subject selection
            // But better: let's filter client side if backend doesn't support "has_subject=true"

            const data = await attendanceService.getAttendanceRecords(schoolCode, params);

            // Client-side filter to ensure we only show subject attendance (records with subject_id)
            const subjectRecords = data.filter(r => r.subject_id !== null);
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const headers = ['Date', 'Time', 'Time of Day', 'Student', 'Class', 'Subject', 'Status', 'Marked By', 'Notes'];
            ...
            link.setAttribute('download', `subject_attendance_${new Date().toISOString().split('T')[0]}.csv`);
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);

            showSuccess('Subject attendance records exported successfully');
        } catch (error: any) {
    ...
    return (
        <div className="space-y-4 sm:space-y-6 animate-fade-in pb-6">
            <PageHeader
                ...
                description="View and manage subject-specific attendance history"
            ...
            <DataTable
                ...
                columns={columns}
                loading={loading}
                emptyMessage="No subject attendance records found"
...
export default SubjectAttendanceRecordsPage;
